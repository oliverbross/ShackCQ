...
Morse contract: Android exposes a native Morse training deck with TX groups, callsign copy, and adaptive single-character recognition. The amber instrument face presents the live prompt and scoring truth; settings and user-selected M32 Pocket setup remain secondary. Training must stay local-first and non-transmitting, and M32 USB setup must not be presented as proof of HID, paddle, audio, or RF behaviour.
