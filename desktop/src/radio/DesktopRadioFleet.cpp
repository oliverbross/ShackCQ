...
QVariantList DesktopRadioFleet::descriptors() const {QVariantList result;QStringList ids=m_entries.keys();std::sort(ids.begin(),ids.end());for(const QString &id:ids){const Entry *item=entry(id);const bool connected=item->controller->state().startsWith("Connected");result.push_back(QVariantMap{{"id",item->id},{"name",item->name},{"hamlibModelId",item->modelId},{"manufacturer",item->controller->manufacturer()},{"model",item->controller->model().isEmpty()?item->name:item->controller->model()},{"backend",item->backend},{"connected",connected},{"connection",connected?"live":"offline"},{"capabilities",capabilities(item).toVariantMap()}});}return result;}
