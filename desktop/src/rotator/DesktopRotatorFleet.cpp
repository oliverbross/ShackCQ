...
bool DesktopRotatorFleet::restoreConfiguration(const QVariantMap &section,
                                               QString *error) {
  if (section.value("schemaVersion", 2).toInt() > 2) {
    if (error) *error = "rotatorProfiles schema is newer than supported schema 2";
    return false;
  }
...
void DesktopRotatorFleet::disconnectProfile(const QString &id){if(Entry *item=entry(id)){item->controller->stop();item->controller->disconnectRotator();item->preparationId.clear();++item->generation;emit snapshotChanged();}}
void DesktopRotatorFleet::stopAll(){for(Entry *item:std::as_const(m_entries)){item->controller->stop();item->preparationId.clear();}}
...
QJsonObject DesktopRotatorFleet::snapshot(Entry *item,const QString &agentId,quint64 agentGeneration){
  ...
  const QJsonObject capabilities{{"backend",item->backend},{"model",item->model},{"azimuth",true},{"elevation",item->maximumElevation>item->minimumElevation},{"park",item->backend=="HAMLIB"},{"stop",true},{"minimumAzimuth",item->minimumAzimuth},{"maximumAzimuth",item->maximumAzimuth},{"minimumElevation",item->minimumElevation},{"maximumElevation",item->maximumElevation}};
  return {{"type","rotator.snapshot"},{"protocol",QJsonObject{{"major",1},{"minor",0}}},{"agentId",agentId},{"deviceId",item->id},{"generation",QJsonValue::fromVariant(agentGeneration)},{"sequence",QJsonValue::fromVariant(++item->sequence)},{"observedUtc",QDateTime::currentDateTimeUtc().toString(Qt::ISODateWithMs)},{"connection",connected?(fresh?"live":"stale"):"offline"},{"state",item->preparationId.isEmpty()?"IDLE":"PREPARED"},{"azimuth",fresh?QJsonValue(item->controller->azimuth()):QJsonValue::Null},{"elevation",fresh?QJsonValue(item->controller->elevation()):QJsonValue::Null},{"targetAzimuth",item->preparationId.isEmpty()?QJsonValue::Null:QJsonValue(item->controller->preparedAzimuth())},{"targetElevation",item->preparationId.isEmpty()?QJsonValue::Null:QJsonValue(item->controller->preparedElevation())},{"positionFresh",fresh},{"capabilities",capabilities}};
}
...
QJsonObject DesktopRotatorFleet::processCommand(const QJsonObject &frame,const QString &agentId,quint64 generation){
  ...
  auto result=[&](bool ok,const QString &code){return QJsonObject{{"type","rotator.command.result"},{"protocol",QJsonObject{{"major",1},{"minor",0}}},{"commandId",commandId},{"agentId",agentId},{"deviceId",deviceId},{"generation",QJsonValue::fromVariant(generation)},{"ok",ok},{"code",code}};};
  ...
  if(action=="rotator.stop"){item->controller->stop();item->preparationId.clear();emit snapshotChanged();return result(true,item->controller->positionObserved()?"STOP_REQUESTED_POSITION_PENDING":"STOP_REQUESTED_POSITION_UNKNOWN");}
