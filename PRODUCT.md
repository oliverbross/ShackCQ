...
- Android adds a local-first Morse destination with Morserino-style TX practice, Callsign Trainer, and adaptive Morse Machine. Shared speed/audio/session settings persist locally; generated training audio and practice calls require no account or network. A user-selected M32 Pocket can be configured over USB protocol v1.4 and stream keyed characters into the trainers, while Android keyboard/HID input remains available.
