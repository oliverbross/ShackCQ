...
class QsoDatabase(context: Context, databaseName: String = "shackcq.sqlite") : SQLiteOpenHelper(context, databaseName, null, 17) {
    companion object {
    ...
    fun streamExportADIF(output: java.io.OutputStream, filter: LogbookFilter = LogbookFilter(), stationId: String? = null,
        ...
        output.write("Generated by ShackCQ <ADIF_VER:5>3.1.4 <PROGRAMID:8>ShackCQ <EOH>\n".toByteArray(Charsets.UTF_8))
        var written=0
    ...
    fun toADIF(qso: Qso): String {
        ...
        val fields = listOf(
            ...
            "APP_SHACKCQ_REGION" to qso.region, "CQZ" to qso.cqZone, "ITUZ" to qso.ituZone,
            ...
            "APP_SHACKCQ_DURATION_SECONDS" to qso.durationSeconds.takeIf { it > 0 }?.toString().orEmpty(),
            ...
            "APP_SHACKCQ_QSL_IMAGES" to qso.qslImages)
        ...
        qso.extraAdifFields.toSortedMap().forEach { (rawName, value) ->
            ...
            if (name !in WavelogCanonicalizer.shackCQFields && name !in WavelogCanonicalizer.excludedFields && value.isNotBlank())
                adif = adif.replace("<EOR>", "<$name:${value.toByteArray(Charsets.UTF_8).size}>$value<EOR>")
        }
        return adif
    }
    ...
    fun parseADIF(text: String): Pair<List<Qso>, Int> {
        ...
        Regex("(?is)(.*?<EOR>)").findAll(text.substringAfter("<EOH>", text)).forEach { match ->
            ...
            val extras = parsed.filterKeys { it !in WavelogCanonicalizer.shackCQFields && it !in WavelogCanonicalizer.excludedFields }
    ...
    fun qsoFromFields(field: (String) -> String, remoteId: String, stationProfileId: String,
        ...
        return Qso(
            id = field("APP_KX3TOUCH_UUID").ifBlank { field("APP_SHACKCQ_UUID") }
            ...
            dxcc = field("DXCC"), continent = field("CONT"), region = field("APP_SHACKCQ_REGION"),
            ...
            durationSeconds = field("APP_SHACKCQ_DURATION_SECONDS").toLongOrNull()
            ...
            qslImages = field("APP_SHACKCQ_QSL_IMAGES"), syncState = if (remoteId.isBlank()) "local" else "synced",
