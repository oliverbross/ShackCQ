...
import app.shackcq.mobile.keyer.KeyerQueueState
...
internal fun buildSystemHealthSnapshot(
    ...
    val bytes = mapOf(
        "shackcq.sqlite" to (qso?.databaseBytes ?: databaseBytes("shackcq.sqlite")),
        "neural-dx.sqlite" to databaseBytes("neural-dx.sqlite"),
        "shackcq-digi.sqlite" to databaseBytes("shackcq-digi.sqlite"),
        "shackcq-groupsio.sqlite" to databaseBytes("shackcq-groupsio.sqlite"),
        "shackcq-contest.sqlite" to databaseBytes("shackcq-contest.sqlite"),
        "shackcq-dxchaser.sqlite" to databaseBytes("shackcq-dxchaser.sqlite"),
    )
...
object SanitizedSupportBundle {
    fun build(snapshot: SystemHealthSnapshot, upstreamPins: Map<String, String>, packageSummary: Map<String, Long>): ByteArray {
        val root = JSONObject()
            .put("format", "SHACKCQ_SANITIZED_SUPPORT_V1")
        ...
        return ByteArrayOutputStream().use { output ->
            ZipOutputStream(output).use { zip ->
                zip.putNextEntry(ZipEntry("shackcq-support.json"))
