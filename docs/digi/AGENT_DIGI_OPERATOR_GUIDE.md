...
Do not enable either TX gate on this review branch. Hamlib work now runs behind a primary-process plus independent emergency STOP/watchdog boundary with bounded retry, but authoritative UTC quality is not wired, real-device audio conversion is not accepted, and physical-radio STOP/RX timing has not been verified. Any `RX_UNCONFIRMED` result remains fail-closed and requires local RX evidence before reconnecting. The current slotted-mode arm path intentionally returns `CLOCK_QUALITY_UNVERIFIED`.
